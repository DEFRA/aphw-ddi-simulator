# DDI One Login Simulator (forked from GOV.UK One Login Simulator https://github.com/govuk-one-login/simulator)

## Quickstart

You will need to copy ``.env.example`` to ``.env`` and put your email address in.  Then run the following command:

```
./scripts/start
```

``aphw-ddi-api`` will need to have the following environment variables:

```shell
# One login stub
export AUTH_SERVER_HOSTNAME="http://host.docker.internal:8443"
...
```shell
#  Setup for Stub
export OPENID_CLIENT_ID=HGIOgho9HIRhgoepdIOPFdIUWgewi0jw
export OPENID_DISCOVERY_ENDPOINT=http://host.docker.internal:8443/.well-known/openid-configuration
export OPENID_PRIVATE_KEY="MIIEvwIBADANBgkqhkiG9w0BAQEFAASCBKkwggSlAgEAAoIBAQCZddHcSxG9QxWE
