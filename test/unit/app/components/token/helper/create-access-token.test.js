...
const { createAccessToken } = require('../../../../../../app/components/token/helper/create-access-token')
...
describe('createAccessToken tests', () => {
  ...
  test('does not contain any claims if no claims were given', async () => {
    const vtr = {
      credentialTrust: 'Cl.Cm',
      levelOfConfidence: 'P2'
    }
  ...
  test('does not contain any claims if level of confidence is null', async () => {
    const vtr = {
      credentialTrust: 'Cl',
