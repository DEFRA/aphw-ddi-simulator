const { exportSPKI, SignJWT } = require('jose')
const { Config } = require('../../../../app/config')
const { ParseTokenRequestError } = require('../../../../app/errors/parse-token-request-error')
const { TokenRequestError } = require('../../../../app/errors/token-request-error')
const { parseTokenRequest } = require('../../../../app/parse/parse-token-request')
...
describe('parseTokenRequest tests', () => {
  ...
  test('throws an invalid_request error for no grant_type', async () => {
    await expect(
      parseTokenRequest(
        {
          code: '1234',
  ...
  test('throws an invalid_request error when the grant_type is not authorization_code', async () => {
    await expect(() =>
      parseTokenRequest(
        {
  ...
  test('throws an invalid_request error when no redirect_uri is included', async () => {
    await expect(() =>
      parseTokenRequest(
        {
  ...
  test('throws an invalid_request error when no code is included', async () => {
    await expect(
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an invalid_request error when the client_assertion_type is not included', async () => {
    await expect(
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an invalid_request error when the client_assertion is not included', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an invalid when the client_assertion_type is not urn:ietf:params:oauth:client-assertion-type:jwt-bearer', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError for an invalid JWS in the client_assertion', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if the payload is invalid JSON', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if the signature is empty', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if there is no sub claim in the client_assertion', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if there is no issuer in the client_assertion', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if there is no aud in the client_assertion', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if there is no exp in the client_assertion', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if the exp is invalid the client_assertion', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if the nbf claim is invalid the client_assertion', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if there is no alg in the header', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an ParseTokenRequestError if the alg in the header is not RS256 or ES256', async () => {
    await expect(() =>
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an error if the token request includes a top-level client_id which does not match the sub in the client_assertion', async () => {
    await expect(
      parseTokenRequest(
        {
          grant_type: 'authorization_code',
  ...
  test('throws an invalid_client error if the client_id from the client_assertion does not match the config', async () => {
  ...
  test('throws an invalid_grant error for an expired client_assertion', async () => {
    clientIdSpy.mockReturnValue(knownClientId)
    const publicKey = await exportSPKI(rsaKeyPair.publicKey)
    publicKeySpy.mockReturnValue(publicKey)
    idTokenSigningSpy.mockReturnValue('RS256')
  ...
  test('throws an invalid_client error if the client_assertion aud is not the expected token endpoint', async () => {
    clientIdSpy.mockReturnValue(knownClientId)
    const publicKey = await exportSPKI(rsaKeyPair.publicKey)
    publicKeySpy.mockReturnValue(publicKey)
    idTokenSigningSpy.mockReturnValue('RS256')
  ...
  test('throws an invalid_client error if the client_assertion iss and sub claims do not match', async () => {
    clientIdSpy.mockReturnValue(knownClientId)
    const publicKey = await exportSPKI(rsaKeyPair.publicKey)
    publicKeySpy.mockReturnValue(publicKey)
    idTokenSigningSpy.mockReturnValue('RS256')
  ...
  test('throws an invalid_client signature error if the signature cannot be validated', async () => {
    clientIdSpy.mockReturnValue(knownClientId)
    const publicKey = await exportSPKI(rsaKeyPair.publicKey)
    publicKeySpy.mockReturnValue(publicKey)
    idTokenSigningSpy.mockReturnValue('RS256')
  ...
  test('returns a parsed tokenRequest and clientAssertion for a valid request', async () => {
    clientIdSpy.mockReturnValue(knownClientId)
    const publicKey = await exportSPKI(rsaKeyPair.publicKey)
    publicKeySpy.mockReturnValue(publicKey)
    idTokenSigningSpy.mockReturnValue('RS256')
  ...
  test('returns a parsed tokenRequest and clientAssertion for a valid request when no affixes on public key', async () => {
    clientIdSpy.mockReturnValue(knownClientId)
    const publicKey = await exportSPKI(rsaKeyPair.publicKey)
